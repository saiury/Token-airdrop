// Generated by CoffeeScript 2.3.2
var Eth, abi, axios, bn, delay, ether, fs, keys, log, logs, logs2, send, vesting_logs, vesting_send, wei;

keys = require('../keys.js');

fs = require('fs').promises;

axios = require('axios');

Eth = require('ethjs');

abi = require('ethjs-abi');

log = console.log;

ether = function(val) {
  return Eth.fromWei(val, 'ether');
};

wei = function(val) {
  return Eth.toWei(val, 'ether');
};

bn = function(val) {
  return new Eth.BN(val, 10);
};

delay = function(ms) {
  return new Promise((resolve, reject) => {
    return setTimeout(resolve, ms);
  });
};


const getNonce = account => new Promise((resolve, reject) => {
	web3.eth.getTransactionCount(account, (err, result) => {

		if(err)
		{
			return reject(err);
		}

		resolve(result);
	});
});
;

send = async function() {
  var amounts, arr, data, err, i, ids, j, k, l, nonce, receivers, ref, ref1, res, sender, sender_addr, size, val;
  sender = artifacts.require('MassSender');
  sender_addr = '0xcdbB5a2D305f179fcfF384499cDef4D6265B3082'.toLowerCase();
  try {
    res = (await fs.readFile('./data/5/investors_2.dat'));
    res = String(res);
    arr = res.split('\n');
  } catch (error1) {
    err = error1;
    log(err);
    return;
  }
  //	data = []
  //	for val in arr
  //		tmp = val.split '\t'
  //		data.push {id: tmp[0], addr: tmp[1], amount: wei(tmp[2]).toString()}
  data = [
    {
      id: 3100,
      addr: '0x246c8e972a0FbDEF6896D97a993F8B54DD2a215C'.toLowerCase(),
      amount: wei('17815.9')
    }
  ];
  //	sender = await sender.deployed()
  sender = (await sender.at(sender_addr));
  nonce = (await getNonce(keys.owner));
  size = 2;
  for (i = k = 0, ref = data.length, ref1 = size; ref1 !== 0 && (ref1 > 0 ? k < ref : k > ref); i = k += ref1) {
    arr = data.slice(i, i + size);
    ids = (function() {
      var l, len, results;
      results = [];
      for (l = 0, len = arr.length; l < len; l++) {
        val = arr[l];
        results.push(val.id);
      }
      return results;
    })();
    receivers = (function() {
      var l, len, results;
      results = [];
      for (l = 0, len = arr.length; l < len; l++) {
        val = arr[l];
        results.push(val.addr);
      }
      return results;
    })();
    amounts = (function() {
      var l, len, results;
      results = [];
      for (l = 0, len = arr.length; l < len; l++) {
        val = arr[l];
        results.push(val.amount);
      }
      return results;
    })();
    for (j = l = 0; l <= 2; j = ++l) {
      res = (await sender.bulkTransfer('0xbf799a2f71d020a4a8c10e7406e2bf970b3d734b', ids, receivers, amounts, {nonce}));
      log(res);
      nonce += 1;
    }
  }
  return (await delay(1000));
};

vesting_send = async function() {
  var amounts, arr, data, err, i, ids, j, k, l, len, nonce, receivers, ref, ref1, res, results, sender, sender_addr, size, tmp, val, vestings;
  sender = artifacts.require('MassVestingSender');
  sender_addr = '0xCBC66115e9d8655709c3408D0e320410Aef1161A'.toLowerCase();
  try {
    res = (await fs.readFile('./data/7/vesting.dat'));
    res = String(res);
    arr = res.split('\n');
  } catch (error1) {
    err = error1;
    log(err);
    return;
  }
  data = [];
  for (k = 0, len = arr.length; k < len; k++) {
    val = arr[k];
    tmp = val.split('\t');
    data.push({
      id: tmp[0],
      addr: tmp[2],
      amount: wei(tmp[3]).toString(),
      vesting: tmp[4]
    });
  }
  data = [
    {
      id: 1801,
      addr: '0x246c8e972a0FbDEF6896D97a993F8B54DD2a215C'.toLowerCase(),
      amount: wei('20469.99999'),
      vesting: '1546112999'
    }
  ];
  //#	sender = await sender.deployed()

  //	0xb901bdbcfe5470b854c4434b987e29ea1290dd5a
  //	0x1b68e9b0dc65d5822e078667184cc0fa64b4931f
  //	0xc6f695202eb1190f53a6e2b32351e1f2ef4ab78e
  sender = (await sender.at(sender_addr));
  nonce = (await getNonce(keys.owner));
  size = 5;
  results = [];
  for (i = l = 0, ref = data.length, ref1 = size; ref1 !== 0 && (ref1 > 0 ? l < ref : l > ref); i = l += ref1) {
    arr = data.slice(i, i + size);
    ids = (function() {
      var len1, m, results1;
      results1 = [];
      for (m = 0, len1 = arr.length; m < len1; m++) {
        val = arr[m];
        results1.push(val.id);
      }
      return results1;
    })();
    receivers = (function() {
      var len1, m, results1;
      results1 = [];
      for (m = 0, len1 = arr.length; m < len1; m++) {
        val = arr[m];
        results1.push(val.addr.toLowerCase());
      }
      return results1;
    })();
    amounts = (function() {
      var len1, m, results1;
      results1 = [];
      for (m = 0, len1 = arr.length; m < len1; m++) {
        val = arr[m];
        results1.push(val.amount);
      }
      return results1;
    })();
    vestings = (function() {
      var len1, m, results1;
      results1 = [];
      for (m = 0, len1 = arr.length; m < len1; m++) {
        val = arr[m];
        results1.push(val.vesting);
      }
      return results1;
    })();
    results.push((await (async function() {
      var m, results1;
      results1 = [];
      for (j = m = 0; m < 2; j = ++m) {
        try {
          res = (await sender.bulkTransfer('0xbf799a2f71d020a4a8c10e7406e2bf970b3d734b', ids, receivers, amounts, vestings, {nonce}));
          log(res);
        } catch (error1) {
          err = error1;
          log(err);
        }
        results1.push(nonce += 1);
      }
      return results1;
    })()));
  }
  return results;
};

vesting_logs = async function(res_file_name) {
  var data, sender, sender_addr, vesting_transfer;
  try {
    sender = artifacts.require('MassVestingSender');
    sender_addr = '0xCBC66115e9d8655709c3408D0e320410Aef1161A'.toLowerCase();
    //	sender = await sender.deployed()
    sender = (await sender.at(sender_addr));
    data = [];
    vesting_transfer = sender.VestingTransfer({}, {
      fromBlock: 0,
      toBlock: 'latest'
    });
    return vesting_transfer.get(async(error, logs) => {
      var k, l, len, len1, res, val;
      for (k = 0, len = logs.length; k < len; k++) {
        val = logs[k];
        if (val.args._recipient !== '0xacE624812a8E2d8Ff0112784BBB3dE26355b0A3B'.toLowerCase()) {
          if (val.args._recipient !== '0xF9c024A74938F8e9172C14dE7b513D3ce4329B49'.toLowerCase()) {
            data.push({
              recipient: val.args._recipient,
              lock: val.args._lock,
              amount: ether(val.args._amount),
              vesting: val.args._vesting.toString()
            });
          }
        }
      }
      res = 'recipient;lock;amount;vesting\r\n';
      for (l = 0, len1 = data.length; l < len1; l++) {
        val = data[l];
        res += `${val.recipient};${val.lock};${val.amount};${val.vesting}\r\n`;
      }
      log(data.length);
      await fs.writeFile(res_file_name, res);
      log('cmpl');
    });
  } catch (error1) {}
};

logs = async function(res_file_name) {
  var addr, block, data, k, len, max, ref, res, sender_addr, val;
  addr = '0xbf799a2f71d020a4a8c10e7406e2bf970b3d734b'.toLowerCase();
  sender_addr = '0x000000000000000000000000cdbb5a2d305f179fcff384499cdef4d6265b3082'.toLowerCase();
  block = 6952961;
  max = 6975700;
  res = 'recipient;amount\r\n';
  while (block < max) {
    data = (await axios.get(`https://api.etherscan.io/api?module=logs&action=getLogs&fromBlock=${block}&toBlock=${block + 10}&address=${addr}&topic1=${sender_addr}&apikey=${keys.APIKEY}`));
    ref = data.data.result;
    for (k = 0, len = ref.length; k < len; k++) {
      val = ref[k];
      res += `0x${val.topics[2].slice(26)};${abi.decodeParams(['uint'], val.data)[0].toString()}\r\n`;
    }
    log(block, data.statusText);
    block = block + 11;
    await delay(1000);
  }
  await fs.writeFile(res_file_name, res);
};

logs2 = async function() {
  var arr, k, len, res, tmp, val;
  res = (await fs.readFile('./data/all_log.csv'));
  res = String(res);
  arr = res.split('\r\n');
  res = arr[0] + '\r\n';
  arr = arr.slice(1);
  for (k = 0, len = arr.length; k < len; k++) {
    val = arr[k];
    tmp = val.split(';');
    res += `${tmp[0]};${ether(tmp[1])}\r\n`;
  }
  await fs.writeFile('./data/all_log2.csv', res);
};

//module.exports = (cb)->
//logs './data/all_log.csv'

//	vesting_send()

//logs './data/all_log.csv'
logs2();

//# sourceMappingURL=sender.js.map
